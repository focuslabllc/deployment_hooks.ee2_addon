...
class Deployment_hooks_mcp {
	...
	public function index()
	{
		...
		foreach ($hooks as $which)
		{
			...
			$base_url = (empty($this->_settings['dh:http_auth'])) ? $this->_EE->config->item('base_url') : str_replace('http://', 'http://username:password@', $this->_EE->config->item('base_url'));

			// Build our pre and post action urls
			$data[$which.'_action_url']  = $base_url
			                             . QUERY_MARKER
			                             . 'ACT='
	...
	private function _security_checks()
	{
		...
		if (isset($this->_settings['dh:http_auth']))
		{
			$this->_EE->load->library('template', NULL, 'TMPL');
			$this->_EE->load->model('member_model');

			if ( ! isset($this->_settings['dh:http_auth_member_groups']))
			{
				$this->_settings['dh:http_auth_member_groups'] = array();
			}
			...
			{
				if ( ! in_array($row->group_id, $this->_settings['dh:http_auth_member_groups']))
				{
					$not_allowed_groups[] = $row->group_id;
				}
			}

			$pass = $this->_EE->TMPL->template_authentication_check_basic($not_allowed_groups);

			if ($pass === FALSE)
			{
				$this->response[] = lang('dh:http_auth_failed');
			}
		}
